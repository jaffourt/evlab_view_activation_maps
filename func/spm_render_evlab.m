function varargout = spm_render_evlab(dat,brt,rendfile,p_val)
...
elseif nargin < 1
    ...
    if isstruct(dat)
        ...
        for i=1:num
            V  = spm_vol(files{i});
            d  = spm_read_vols(V);
            u=spm_u(p_val^(1/1),[1.000,326.000],'T');
            Q = find(d > u);
            %id = find(isfinite(d));
            [X,Y,Z]    = ind2sub(V.dim,Q);
            dat(i).XYZ = [X Y Z]';
            dat(i).t   = d(Q);
            dat(i).mat = V.mat;
            dat(i).dim = V.dim';
...
%==========================================================================
if ~spm('CmdLine')
    Fgraph = spm_figure('Create','temp_tag1','temp_tag2','off');
